...
data['compound_steps'] = st.sidebar.slider("Compounding Steps", 1, 10, int(data['compound_steps']))
data['payout_pct'] = st.sidebar.number_input("Payout % on WIN", value=data['payout_pct'], step=5.0)
data['profit_target_pct'] = st.sidebar.number_input("Daily Profit Target (%)", value=data['profit_target_pct'], step=0.5)
data['loss_target_pct'] = st.sidebar.number_input("Daily Loss Target (%)", value=data['loss_target_pct'], step=0.5)
...
st.markdown("**Advanced Compounding Cycle System**")
...
with col3:
    st.metric("Cycle / Step", f"{data['current_cycle']} | {data['current_step']}/{data['compound_steps']}")
with col4:
...
with col_a:
    next_amount = data['base_amount'] if data['current_step'] == 1 or not data['trade_history'] else \
                  (data['trade_history'][-1]['trade_amount'] + data['trade_history'][-1]['profit'])
    st.info(f"**Next Trade Amount:** ৳{next_amount:,.2f}")
with col_b:
    st.info(f"**Payout:** {data['payout_pct']}% | **Profit Target:** {data['profit_target_pct']}% | **Loss Target:** {data['loss_target_pct']}%")
